...
export const togglePayment = async (req, res) => {
  try {
    ...
    const settings = await settingsmodel.findOne({})
    if (!settings) return res.json({ success: false, message: "Settings not found" })
...
// ── Update delivery settings ── NEW
export const updateDelivery = async (req, res) => {
  try {
    const { deliveryFee, freeDeliveryAbove } = req.body

    if (deliveryFee === undefined && freeDeliveryAbove === undefined) {
      return res.json({ success: false, message: "No delivery fields provided" })
    }

    const updateData = {}
    if (deliveryFee       !== undefined) updateData.deliveryFee       = Number(deliveryFee)
    if (freeDeliveryAbove !== undefined) updateData.freeDeliveryAbove = Number(freeDeliveryAbove)

    const settings = await settingsmodel.findOneAndUpdate(
      {}, updateData, { new: true, upsert: true }
    )
    res.json({ success: true, message: "Delivery settings updated", settings })
  } catch (error) {
    console.log(error)
    res.json({ success: false, message: error.message })
  }
...
export const changeOwnPassword = async (req, res) => {
  try {
    ...
    const isSame = await bcrypt.compare(newPassword, admin.password)
    if (isSame) return res.json({ success: false, message: "New password must be different" })

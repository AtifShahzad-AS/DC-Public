...
//         { headers: { Authorization: `Bearer ${token}` } }
//       )
//       if (data.success) setHasReviewed(data.hasReviewed)
//     } catch (err) {
//       console.log('Check review error:', err)
//     }
//   }
...
//   // ── Handle add to cart with brief feedback ──
...
//               <img
//                 className={`w-full h-auto rounded-xl object-cover ${isOutOfStock ? 'opacity-70' : ''}`}
//                 src={image}
//                 alt={productdata.name}
//               />
...
//                   <span className="bg-white border-2 border-red-300 text-red-500 font-bold text-lg px-6 py-2 rounded-full shadow">
//                     Out of Stock
//                   </span>
//                 </div>
//               )}
...
//                   Out of Stock
//                 </span>
//               ) : productdata.stock <= (productdata.lowStockAlert || 5) ? (
...
//                 </span>
//               ) : (
//                 <span className="inline-flex items-center gap-1.5 bg-green-50 border border-green-200 text-green-600 text-xs font-semibold px-3 py-1.5 rounded-full">
...
//                 <button
//                   disabled
//                   className="bg-gray-100 text-gray-400 border border-gray-200 px-10 py-3 rounded-xl text-sm font-semibold cursor-not-allowed w-full sm:w-auto"
//                 >
//                   Out of Stock
//                 </button>
//               ) : (
...
//                   className={`px-10 py-3 rounded-xl text-sm font-semibold transition-all duration-200 active:scale-95
//                     ${addedToCart
  ...
  <div className="flex gap-1">
    {[1, 2, 3, 4, 5].map(s => (
      <button key={s} type={interactive ? 'button' : undefined}
        onClick={() => interactive && onChange && onChange(s)}
        className={interactive ? 'cursor-pointer' : 'cursor-default'}>
        <svg className={`w-4 h-4 ${s <= rating ? 'text-yellow-400' : 'text-slate-200'}`}
          fill="currentColor" viewBox="0 0 20 20">
          <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
        </svg>
      </button>
...
const Product = () => {
  ...
  const fetchReviews = async () => {
    ...
    try {
      const { data } = await axios.post(backendurl + '/api/review/list', { productId: productid })
      if (data.success) setReviews(data.reviews)
    } catch (err) { console.log('Reviews not available yet') }
  ...
  useEffect(() => { fetchproductdata() }, [productid])
  useEffect(() => {
    if (activeTab === 'reviews') { fetchReviews(); checkUserReview() }
  }, [activeTab, productid, token])

  const hasSizes     = productdata?.sizes && productdata.sizes.length > 0
  const isOutOfStock = productdata?.stock === 0
  const onSale       = productdata?.onSale && productdata?.salePrice > 0
  ...
  const avgRating = reviews.length > 0
    ? (reviews.reduce((sum, r) => sum + r.rating, 0) / reviews.length).toFixed(1)
    : productdata?.rating > 0 ? productdata.rating.toFixed(1) : null
      ...
      <div className='border-t-2 pt-10 transition-opacity ease-in duration-500 opacity-100'>
        ...
        <div className='flex gap-10 flex-col sm:flex-row'>
          ...
          <div className='flex-1 flex flex-col-reverse gap-3 sm:flex-row'>
            <div className="flex flex-row sm:flex-col overflow-x-auto sm:overflow-y-auto gap-2 sm:w-[18%]">
              {productdata.image.map((item, index) => (
                <img key={index} onClick={() => setimage(item)} src={item}
                  className={`w-20 sm:w-full flex-shrink-0 cursor-pointer rounded-lg border-2 transition-all duration-200
                    ${image === item ? 'border-blue-600' : 'border-transparent hover:border-slate-300'}`}
                  alt="" />
              ))}
            ...
            <div className='w-full sm:w-[80%] relative'>
              <img className={`w-full h-auto rounded-xl object-cover ${isOutOfStock ? 'opacity-70' : ''}`}
          ...
          {/* ── Product Info ── */}
          <div className="flex-1">
            <h1 className='font-bold text-2xl sm:text-3xl text-slate-900 mt-2'>{productdata.name}</h1>
              ...
              {onSale ? (
                <div className="flex items-center gap-3">
                  <p className='text-3xl font-bold text-red-500'>
                    {currency}{productdata.salePrice.toLocaleString()}
                  </p>
            ...
            <p className='mt-4 text-slate-500 leading-relaxed md:w-4/5'>{productdata.description}</p>
            ...
            <div className="mt-4">
              {isOutOfStock ? (
                ...
                <span className="inline-flex items-center gap-1.5 bg-orange-50 border border-orange-200 text-orange-600 text-xs font-semibold px-3 py-1.5 rounded-full">
                  <span className="w-1.5 h-1.5 rounded-full bg-orange-500 inline-block" />Only {productdata.stock} left
                </span>
              ) : (
                <span className="inline-flex items-center gap-1.5 bg-green-50 border border-green-200 text-green-600 text-xs font-semibold px-3 py-1.5 rounded-full">
            ...
            <div className="mt-6">
              {isOutOfStock ? (
                <button disabled className="bg-gray-100 text-gray-400 border border-gray-200 px-10 py-3 rounded-xl text-sm font-semibold cursor-not-allowed w-full sm:w-auto">
                  Out of Stock
                </button>
              ) : (
            ...
            {/* Trust badges */}
            <div className='text-sm text-slate-500 mt-5 flex flex-col gap-2'>
              {['100% original product', 'Cash on delivery available', 'Easy return and exchange policy'].map(text => (
                <div key={text} className="flex items-center gap-2">
                  <svg className="w-4 h-4 text-green-500 flex-shrink-0" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                    <path d="M22 11.08V12a10 10 0 11-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/>
                  </svg>
                  ...
                  <form onSubmit={handleSubmitReview} className="space-y-4">
                    <div>
                      ...
                      <StarRating rating={reviewForm.rating}
                        onChange={val => setReviewForm(prev => ({ ...prev, rating: val }))} interactive />
                    </div>
                    <div>
                      <p className="text-xs text-slate-500 mb-1.5">Your Review</p>

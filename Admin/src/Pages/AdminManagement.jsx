...
//         { headers: { token } }
//       )
//       if (data.success) toast.success('Delivery settings saved')
...
//               outline-none focus:border-[#e8a87c] bg-gray-50"
//           />
...
//         Orders under{' '}
//         <strong>Rs {Number(freeDeliveryAbove).toLocaleString()}</strong>
//         {' '}→ delivery fee{' '}
//         <strong>Rs {Number(deliveryFee).toLocaleString()}</strong>
//         {' · '}
//         Orders above → <strong>Free delivery</strong>
//       </div>

//       <button
//         onClick={handleSave}
//         disabled={loading}
//         className="bg-[#1a1a2e] text-white text-xs font-medium px-5 py-2.5
//           rounded-lg hover:bg-[#2a2a3e] transition-colors disabled:opacity-60"
//       >
...
//             <h2 className="text-sm font-semibold text-[#1a1a2e]">Admin Accounts</h2>
//             <p className="text-xs text-gray-400 mt-0.5">
//               Only super admin can create or deactivate admins
//             </p>
//           </div>
//           <button
//             onClick={() => setShowForm(!showForm)}
...
//                     onChange={e => setEmail(e.target.value)}
//                     placeholder="sara@diamondcollection.pk" required
//                     className="w-full border border-gray-200 rounded-lg px-3 py-2 text-xs text-[#1a1a2e] outline-none focus:border-[#e8a87c] bg-white"
//                   />
//                 </div>
//                 <div className="sm:col-span-2">
    ...
    <div className="space-y-5">
      ...
      <div className="bg-white rounded-xl border border-black/5 overflow-hidden">
        <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
          <div>
            <h2 className="text-sm font-semibold text-[#1a1a2e]">Admin Accounts</h2>
            <p className="text-xs text-gray-400 mt-0.5">Super admin access required for changes</p>
          </div>
          <button
            onClick={() => setShowForm(!showForm)}
            className="flex items-center gap-1.5 bg-[#1a1a2e] text-white text-xs font-medium px-3.5 py-2 rounded-lg hover:bg-[#2a2a3e] transition-colors"
            ...
            <form onSubmit={handleCreate} className="space-y-3">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <input
                  ...
                  className="w-full border border-gray-200 rounded-lg px-3 py-2 text-xs outline-none focus:border-[#e8a87c]"
                />
                <input
                  type="email" value={email}
                  onChange={e => setEmail(e.target.value)}
                ...
                <div className="sm:col-span-2 relative">
                  ...
                  <button 
                    type="button" 
                    onClick={() => setShowPassword(!showPassword)}
                    className="absolute right-3 top-2 text-gray-400 text-[10px]"
                  >
              ...
              <div className="flex gap-2">
                <button type="submit" disabled={creating} className="bg-[#e8a87c] text-white text-xs px-4 py-2 rounded-lg">
                  {creating ? 'Creating...' : 'Create Admin'}
                </button>
                <button type="button" onClick={() => setShowForm(false)} className="text-xs px-4 py-2">Cancel</button>
              </div>
            </form>
          </div>
        )}
        ...
        {/* Admins List */}
        {loading ? (
          <div className="py-10 text-center text-xs text-gray-400">Loading admins...</div>
        ) : (
          <div className="divide-y divide-gray-50">
            {admins.map((admin, i) => {
              const av = avatarColors[i % avatarColors.length]
              return (
                <div key={admin._id} className="px-5 py-4 flex items-center justify-between hover:bg-gray-50/50">
                  <div className="flex items-center gap-3">
                    <div className="w-9 h-9 rounded-full flex items-center justify-center text-xs font-semibold" style={{ background: av.bg, color: av.text }}>
                      {initials(admin.name)}
                    </div>
                    <div>
                      <div className="flex items-center gap-2">
                        <p className="text-xs font-semibold text-[#1a1a2e]">{admin.name}</p>
                        <span className={`text-[10px] px-2 py-0.5 rounded-full ${roleBadge(admin.role)}`}>{admin.role}</span>
                      </div>
                  ...
                  {admin.role !== 'superadmin' && (
                    <button
                      onClick={() => handleToggle(admin._id)}
                      className={`text-[11px] px-3 py-1.5 rounded-lg border ${admin.isActive ? 'text-gray-500' : 'text-green-700 bg-green-50 border-green-200'}`}
                    >
                      {admin.isActive ? 'Deactivate' : 'Activate'}
                    </button>
                  )}

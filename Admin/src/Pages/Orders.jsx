...
const Orders = ({ token }) => {
  ...
  const [freeAbove, setFreeAbove]         = useState(3000)
    ...
    const fetchSettings = async () => {
      try {
        ...
        if (data.success) {
          ...
          setFreeAbove(data.settings.freeDeliveryAbove ?? 3000)
        }
  ...
  const statushandler = async (event, orderId) => {
    try {
      ...
      if (response.data.success) await fetchorder()
    } catch (error) {
      console.log(error)
      toast.error(error.message)
    }
  ...
  // ── Calculate subtotal from items (excludes delivery) ──
  const getSubtotal = (order) => {
    return order.items.reduce((sum, item) => sum + (item.price * item.quantity), 0)
  }

  // ── Get delivery fee for this order ──
  // Old orders had Rs 10 hardcoded — detect by checking if amount - subtotal is suspiciously small
  const getOrderDeliveryFee = (order) => {
    const subtotal = getSubtotal(order)
    const diff     = order.amount - subtotal
    // if diff is 0 or negative → free delivery or already correct
    // if diff is > 0 → that's the delivery fee that was charged
    ...
    <div className='w-full px-3 sm:px-6 lg:px-8 py-5 max-w-5xl mx-auto'>
      ...
      <div className='flex flex-col gap-3'>
        {orders.map((order, index) => {
          const cfg        = statusConfig[order.status] || { bg: '#F3F4F6', color: '#374151', dot: '#6B7280' }
          const subtotal   = getSubtotal(order)
          const orderFee   = getOrderDeliveryFee(order)

          // ── Amount display: show breakdown ──
          const AmountBlock = () => (
                ...
                <div className='flex items-center justify-between'>
                  ...
                  <div className='flex items-center gap-1.5'>
                    <div>
                      <p className='text-sm font-bold text-gray-900'>{currency}{order.amount}</p>
                      {orderFee > 0 && (
                        <p className='text-[9px] text-gray-400'>+{currency}{orderFee} delivery</p>
                      )}
                      {orderFee === 0 && subtotal > 0 && (
                        <p className='text-[9px] text-green-600'>Free delivery</p>
                      )}
                    </div>
                    <span className='text-[10px] font-semibold rounded-full px-2 py-0.5'
                      style={{ background: order.payment ? '#DCFCE7' : '#FEF9C3', color: order.payment ? '#15803D' : '#A16207' }}>
